...
const selectData = {
  assignee: [
    {id: selectKey.intransit, label: 'In Transit'},
...
const NBAUnloadingDetailScreen = ({
  route,
  navigation,
  loading,
  startLoading,
  ...
  const [status, setStatus] = useState(selectKey.intransit);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* <Pressable
        style={[styles.fullImageArea, {zIndex: fullImage ? 999 : -999}]}
      ...
      <View
        ...
        <View
          ...
          <TextInput
            ...
            bordered
            placeholder="Typing..."
            value={truckDetail.vhclRemarks}
            onChangeText={val =>
              setTruckDetail({...truckDetail, vhclRemarks: val})
            }
          />
        </View>
        ...
        <ScrollView horizontal={true} style={{marginVertical: 8}}>
          {files?.map((it, id) => (
            <Pressable
              ...
              key={id.toString()}>
              <Image
                source={{uri: genImageSource(it?.fileId)}}
                defaultSource={defaultImage}
